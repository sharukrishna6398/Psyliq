...
select distinct month_name from pharma_tb;
select*from pharma_tb;
...
ORDER BY highest_sales DESC
LIMIT 5;
select sub_channel,avg(price) as average_price from pharma_tb
group by sub_channel;
select name_of_the_sales_rep,sum(sales) from pharma_tb group by name_of_the_sales_rep;
select*from pharma_tb where city='Rendsburg' and year_=2018;
